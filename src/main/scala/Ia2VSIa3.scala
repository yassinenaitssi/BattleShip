...
var myBoard = Util.createGrid(Nil , 0)
    ...
    def gamePlay(player: SIA , player1: SIA ,gameState :GameState) : Int =
    {
        ...
        if(replay == 1)
        {
          gamePlay(SIA(0,playerBoard, 0, Set()) , player1 ,GameState(1,0) )            
        }
        else
        {
    ...
    def ia2VSia3(round: Int, totalRound: Int, gameState: GameState, begin: Int, score0 : Int, score1: Int ) : Int =  {
var tr = totalRound
        if(begin == 0)
        {
        ...
        ia2VSia3(round + 1 , tr, GameState(0,0) , 1 , score0 + 10 , score1 )            
        
        }
        else
        {

        ia2VSia3(round + 1 , tr, GameState(0,0) , 1 , score0 , score1 + 10 )   
        
        }
        }
        else
        {
            Proof.recorderIAResult("AI2 ;"  + score0 + "; AI3 ;" + score1)
            Proof.endRecord()
            if(score1 < score0)
            {
                println("======================")
                println("Match winner is number 0")
                println("IA2 score is:" + score0)
